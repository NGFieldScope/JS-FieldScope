...
// ----------------------------------------------------------------------------
// WFSLatLongBoundingBox class

function WFSLatLongBoundingBox (xml) {
...
function WFSFeatureType (xml, version) {
  if (!version) {
    version = "1.0.0";
  }
  ...
  if (version === "1.0.0") {
    ...
    this.bounds = new WFSLatLongBoundingBox(XMLUtils.getFirstElementWithTagName(xml, "LatLongBoundingBox"));
  } else if (version === "1.1.0") {
    srsNode = XMLUtils.getFirstElementWithTagName(xml, "DefaultSRS");
    this.srs = srsNode.text || srsNode.textContent;
    this.bounds = new WFSLatLongBoundingBox(XMLUtils.getFirstElementWithTagName(xml, "WGS84BoundingBox"));
  } else {
...
// ----------------------------------------------------------------------------
// WFSFeatureDescription class

function WFSFeatureDescription (schemaDoc, typeName) {
...
WFSFeatureDescription.prototype.getFormDOM = function () {
...
function WFSCapabilities (baseUrl) {
  var capabilities = null;
  dojo.xhrGet({ url: "StreamingProxy.ashx?url=" +
                ...
                load: function (response, ioArgs) {
                    capabilities = response.documentElement;
                    return response;
                  },
  ...
  this.featureTypes = [];
  for (i = 0; i < types.length; i += 1) {
    this.featureTypes[i] = new WFSFeatureType(types.item(i), version);
  }
}

// ----------------------------------------------------------------------------
// Global function wfsGetFeatureAsync

function wfsGetFeatureAsync (url, layers, bounds, onLoad, onError) {
  return dojo.xhrGet({ 
      url: "StreamingProxy.ashx?url=" +
...
// ----------------------------------------------------------------------------
// Global function wfsDescribeFeatureTypeSync

function wfsDescribeFeatureTypeSync (url, featureType) {
  var xml = null;
  dojo.xhrGet({ 
      url: "StreamingProxy.ashx?url=" +
           encodeURIComponent(url +
  ...
  return new WFSFeatureDescription(xml, featureType);
}

// ----------------------------------------------------------------------------
// WFSLayer class

function WFSLayer (url) {
  
  var capabilities = new WFSCapabilities(url);
  var map = null;
  ...
  function updateShapeLayer () {
      ...
      if (layers.length > 0) {
        ...
        */
        ...
        currentRequest = wfsGetFeatureAsync(capabilities.getFeatureUrl,
                                            layers,
  ...
  this.getFeatureTypeDescription = function (index) {
      if (capabilities.featureTypes[index].description) {
        ...
        var result = wfsDescribeFeatureTypeSync(capabilities.describeFeatureUrl,
...
function wfsInsertAsync (url, features, onInsert, onError) {
