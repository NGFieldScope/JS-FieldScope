...
FieldScope.MetaLens.GDataProvider = function (map, service, url) { 
    ...
    this.CreateInfoWindowHTML = function (data) {
        ...
        if (data.Type === "image") {
          ...
          result +=       '<a href="MetaLensDisplayAsset.aspx?server='+encodeURIComponent(this.url)+'&asset='+encodeURIComponent(assetId)+'"';
          ...
          if (data.Description !== null) {
            ...
            result += '<div style="font-size:9pt;max-height:150px;max-width:256px;word-wrap:break-word;overflow:auto;margin-left:3px">';
        ...
        if (data.Type === "audio") {
          result += '<tr>';
          result +=   '<td>';
          result +=     '<img src="images/dummy.gif" id="FieldScope.MetaLens.ResizeTrigger" />';
          result +=   '</td>';
          result += '</tr>';
        }
        result += '</table>';
